...
resourcegroup_name = 'gx'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'gx'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'westus'
...
coffee.price = 3.99
table_service.insert_entity('itemstable', coffee)
print('Created entry for Starbucks Coffee...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeeshop'", select='brand,flavor,cupsize,price')
for item in items:
    print('Brand: ' + item.brand)
    print('Flavor: ' + item.flavor)
    print('Size of cup: ' + item.cupsize)
    print('Price: ' + str(item.price) + '\n')
